...
namespace NerdStore.Catalogo.Domain.Tests
{
    ...
    {
        [Fact]
        public void Produto_Validar_Validacoes_Devem_Validar_Exceptions()
        {
            // Arrange & Act & Assert
            ...
            ex = Assert.Throws<DomainException>(() =>
                new Produto("Nome", "Descricaooooooooo", false, 100, Guid.NewGuid(), DateTime.Now, "Imagem", new Dimensoes(1, 1, 1))
            );

            Assert.Equal("O campo Descrição passou do limite máximo", ex.Message);
        }
    }
}
