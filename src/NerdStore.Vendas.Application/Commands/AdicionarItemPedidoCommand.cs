...
namespace NerdStore.Vendas.Application.Commands
{
    ...
    // AbstractValidator é uma classe do FluentValidation
    public class AdicionarItemPedidoValidation : AbstractValidator<AdicionarItemPedidoCommand>
    {
        ...
        public AdicionarItemPedidoValidation()
        {
            ...
            RuleFor(c => c.Quantidade).GreaterThan(0).WithMessage("A quantidade mínima de um pedido é 1");

            RuleFor(c => c.Quantidade).LessThan(15).WithMessage("A quantidade máxima de um pedido é 15");

            RuleFor(c => c.ValorUnitario).GreaterThan(0).WithMessage("O valor do item tem que ser maior que 0");
        }
    }
}
